...
var gamePattern = [];
// 4.3. At the top of the game.js file, create a new empty array
// with the name userClickedPattern.
...
$(document).keypress(function () {
  ...
  if (!started) {
    ...
    // 7.3. The h1 title starts out saying "Press A Key to Start",
...
$(".btn").click(function () {
  ...
  // 4.4. Add the contents of the variable userChosenColour created
  // in step 2 to the end of this new userClickedPattern
  userClickedPattern.push(userChosenColour);
  // 5.1. In the same way we played sound in nextSequence() ,
  ...
  // 8.2. Call checkAnswer() after a user has clicked and chosen their answer,
  // passing in the index of the last answer in the user's sequence.
  checkAnswer(userClickedPattern.length-1);
});

// 8.1. Create a new function called checkAnswer(),
// it should take one input with the name currentLevel
function checkAnswer(currentLevel) {
  // 8.3. Write an if statement inside checkAnswer()
  ...
  if (gamePattern[currentLevel] === userClickedPattern[currentLevel]) {
    ...
    // 9.1. In the sounds folder, there is a sound called wrong.mp3,
...
function nextSequence() {
  ...
  // 2.2 Inside the new function generate a new random number between 0 and 3,
  // and store it in a variable called randomNumber
  var randomNumber = Math.floor(Math.random () * 4);
  // 2.4 Create a new variable called randomChosenColour and use the randomNumber from
  ...
  // 3.3 use Javascript to play the sound for the button colour selected in step 1.
  // 5.4. Refactor the code in playSound() so that it will work
  // for both playing sound in nextSequence()
  // and when the user clicks a button.
  playSound(randomChosenColour);
}

// 5.2. Create a new function called playSound()
// that takes a single input parameter called name.
function playSound(name) {
  // 5.3. Take the code we used to play sound in the nextSequence() function
  // and move it to playSound().
  var audio = new Audio('sounds/' + name + '.mp3');
  audio.play();
}

// 6.1. Create a new function called animatePress(),
// it should take a single input parameter called currentColour.
function animatePress(currentColour) {
// 6.2. Take a look inside the styles.css file,
// you can see there is a class called "pressed",
// it will add a box shadow and changes the background colour to grey.
// 6.3. Use jQuery to add this pressed class
